...
impl Rect {
    ...
    pub fn intersect(&self, rect: &Rect) -> bool {
        return self.min[0] < rect.max[0]
            && self.max[0] > rect.min[0]
            && self.max[1] > rect.min[1]
            && self.min[1] < rect.max[1];
    }

    pub fn combine(&self, other: &Rect) -> Rect {
        let mut result = Rect::zero();
        result.min[0] = if self.min[0] < other.min[0] {
            self.min[0]
        } else {
            other.min[0]
        };
        result.min[1] = if self.min[1] < other.min[1] {
            self.min[1]
        } else {
            other.min[1]
        };
        result.max[0] = if self.max[0] > other.max[0] {
            self.max[0]
        } else {
            other.max[0]
        };
        result.max[1] = if self.max[1] > other.max[1] {
            self.max[1]
        } else {
            other.max[1]
        };
...
pub struct UserInterface {
    ...
    immediate_vertex_buffer: Option<Rc<Buffer>>,
    immediate_index_buffer: Option<Rc<Buffer>>,
    vertex_buffer_offset: u64,
    index_buffer_offset: u64,

    gui_texture_bind_group_layout: BindGroupLayout,
    gui_texture_const_group: BindGroup,
    gui_texture_pipeline: RenderPipeline,

    tile_sampler: Sampler,
    default_sampler: Sampler,

    draw_groups: Vec<UIDrawGroup>,

    textures:
    smallvec::SmallVec<[(Arc<TextureResource>, TextureView); RESERVED_TEXTURE_VIEW]>,
}
...
impl UserInterface {
    ...
    pub fn cmd_dispatch(
        ...
        let mut texture_bind_groups: smallvec::SmallVec<[BindGroup; RESERVED_TEXTURE_VIEW]> =
            smallvec::SmallVec::new();
        for texture in self.textures.iter() {
            texture_bind_groups.push(device.create_bind_group(&BindGroupDescriptor {
                entries: &[
                    BindGroupEntry {
                        binding: 0,
                    ...
                    BindGroupEntry {
                        binding: 1,
                        resource: BindingResource::TextureView(&texture.1),
                    },
                ],
...

        {
            ...
            for group in self.draw_groups.iter() {
                match group.get_scissor_rect() {
                    Some(rect) => rpass.set_scissor_rect(
                        rect.min[0] as u32,
                        rect.min[1] as u32,
                        rect.size()[0] as u32,
                        rect.size()[1] as u32,
                    ),
                    _ => rpass.set_scissor_rect(
                        quad.min[0] as u32,
                        quad.min[1] as u32,
                        quad.size()[0] as u32,
                        quad.size()[1] as u32,
                ...
                match group {
                    UIDrawGroup::Texture(ref tex) => {
                        queue.write_buffer(
                            &tex.vertex_buffer.as_ref(),
                            tex.vertex_offset_start,
                            &tex.vertex_shadow_data,
                        );
                        queue.write_buffer(
                            &tex.index_buffer.as_ref(),
                            tex.index_offset_start,
                            &tex.index_shadow_data,
                        );
                        ...
                        rpass.set_index_buffer(
                            tex.index_buffer
                                .slice(tex.index_offset_start..tex.index_offset_end),
                            IndexFormat::Uint32,
                        );
                        rpass.set_vertex_buffer(
                            0,
                            tex.vertex_buffer
                                .slice(tex.vertex_offset_start..tex.vertex_offset_end),
                        );
    ...
    pub fn resize_surface(&mut self, _device: &Device, _size: &IVec2) {
    ...
    fn evaluate_draw_group(&mut self, new_group: UIDrawGroup) -> bool {
        ...
        let group_valid = match self.draw_groups.last() {
            Some(current_draw_group) => match (current_draw_group, &new_group) {
    ...
    fn request_buffer_immediate(
        &mut self,
        device: &Device,
        request_vertex_buffer_size: u64,
        request_index_buffer_size: u64,
    ) -> (u64, u64) {
        let mut vb_last_offset = self.vertex_buffer_offset;
        let mut ib_last_offset = self.index_buffer_offset;

        if self.immediate_vertex_buffer.is_none()
            || (request_vertex_buffer_size + self.vertex_buffer_offset)
            > self.immediate_vertex_buffer.as_ref().unwrap().size()
        {
            let size = self
                .immediate_vertex_buffer
                .as_ref()
                .map_or_else(|| VERTEX_BUFFER_INITIAL_SIZE, |d| d.size() * 2)
                as BufferAddress;
            self.immediate_vertex_buffer =
                Some(Rc::new(device.create_buffer(&BufferDescriptor {
        ...
        if self.immediate_index_buffer.is_none()
            || (request_index_buffer_size + self.index_buffer_offset)
            > self.immediate_index_buffer.as_ref().unwrap().size()
        {
            let size = self
                .immediate_index_buffer
                .as_ref()
                .map_or_else(|| INDEX_BUFFER_INITIAL_SIZE, |d| d.size() * 2)
                as BufferAddress;
            self.immediate_index_buffer =
                Some(Rc::new(device.create_buffer(&BufferDescriptor {
                    label: Some("Unit Square Vertex Buffer"),
                    size,
    ...
    pub fn cmd_draw_texture(
        &mut self,
        _queue: &Queue,
        device: &Device,
        tex: &Arc<TextureResource>,
        uv: &Rect,
        pos: &Rect,
        tint_color: u32,
    ) {
        ...
        let request_vertex_buffer_size =
            (size_of::<PositionTexCoord>() * NUM_VERTS) as u64;
        let request_index_buffer_size = (size_of::<u32>() * NUM_INDCIES) as u64;
        let (vb_buffer_start_offset, ib_buffer_start_offset) = self.request_buffer_immediate(
            device,
            request_vertex_buffer_size,
            request_index_buffer_size,
        );
        ...
        current_group
            .vertex_shadow_data
            .extend_from_slice(bytemuck::cast_slice(vertex_data));
        current_group
            .index_shadow_data
            .extend_from_slice(bytemuck::cast_slice(index_data));
        current_group.vertex_offset_end += request_vertex_buffer_size;
        current_group.index_offset_end += request_index_buffer_size;
    }

    pub fn new(device: &Device, surface: &SurfaceConfiguration) -> UserInterface {
        let gui_per_frame_size = size_of::<GuiTexturePerFrameUniform>() as BufferAddress;

        let gui_texture_shader = device.create_shader_module(ShaderModuleDescriptor {
            label: None,
            source: ShaderSource::Wgsl(Cow::Borrowed(include_str!("gui_texture.wgsl"))),
        });

        let tile_sampler = device.create_sampler(&SamplerDescriptor {
        ...
        let gui_texture_const_bind_group_layout =
        ...
        let gui_texture_bind_group_layout =
            device.create_bind_group_layout(&BindGroupLayoutDescriptor {
                label: None,
                entries: &[
                    BindGroupLayoutEntry {
                        ...
                        ty: BindingType::Buffer {
                            ...
                            min_binding_size: BufferSize::new(gui_per_frame_size),
                        },
                        count: None,
                    },
                    BindGroupLayoutEntry {
        ...
        let gui_texture_const_group = device.create_bind_group(&BindGroupDescriptor {
            entries: &[
                BindGroupEntry {
                    binding: 0,
        ...
        let gui_pipeline_layout = device.create_pipeline_layout(&PipelineLayoutDescriptor {
            label: None,
            bind_group_layouts: &[
                &gui_texture_const_bind_group_layout,
                &gui_texture_bind_group_layout,
            ],
            push_constant_ranges: &[],
        });

        let gui_texture_pipeline = device.create_render_pipeline(&RenderPipelineDescriptor {
